...
output_options = (
    []
)  # ['COMPRESS=LZW', 'TILED=YES'] #, 'COMPRESS=DEFLATE'] also TBD: how to create an overview
...
def Sentinel2SLIC(path_to_mosaic, path_out="output"):
    ...
    print(
        "Satellite scene size is (xs,ys)=(%s,%s) with number of bands is %s"
    ...
    if not slic_zero:
        output_fname = "%s_nseg%s_compact%s_sigma%s" % (
            fname,
            n_segments,
            compactness,
            sigma,
        ...
        output_fname = "%s_SLICO_nseg%s_compact%s_sigma%s" % (
            fname,
            n_segments,
            compactness,
            sigma,
    ...
    slic_ds = out_driver.Create(
        os.path.join(path_out, output_fname + ".tif"),
        segments_slic.shape[1],
        segments_slic.shape[0],
        1,
        out_type,
        output_options,
    )
    ...
    gt_subset = (
        gt[0] + x_ul_px * gt[1],
        gt[1] * x_downscale_factor,
        0,
        gt[3] + y_ul_px * gt[5],
        0,
        gt[5] * y_downscale_factor,
    )
    print(gt_subset)
...
def Sentinel2QuickShift(path_to_mosaic, path_out="outputs"):
    ...
    # converting raster to shape file
    output_fname = "%s_ks%s_md%s_r%s_scale%s_labtrue" % (
        fname,
        kernel_size,
        max_dist,
        ratio,
